...
class BP_Block_User{
    ...
    /*
    *  Restrict blocked users to message
    */
    ...
    */
    public function restrict_blocked(){
           ...
           global $bp;
            // if site admin, skip check
           if( $bp->loggedin_user->is_site_admin == 1 ) return;
    ...
    /*
     * Take care about blocking/unblocking actions
     */
    public function block_user_action(){
        ...
        if(bp_is_user() && !bp_is_my_profile() && is_user_logged_in()){
            ...
            if(isset($_GET['block_user']) &&
                wp_verify_nonce($_GET['_wpnonce'], 'block_user')){
            ...
            if(isset($_GET['unblock_user']) &&
                wp_verify_nonce($_GET['_wpnonce'], 'unblock_user')){
    ...
    public function check_blocking($block_id = false, $user_id = false){
        ...
        //Stop if user not logged or user to block empty
        if($block_id == false || !is_user_logged_in()) return;
        if($user_id == false) $user_id = wp_get_current_user()->ID;

        //Stop if try to block himself
        if($block_id == $user_id) return;
